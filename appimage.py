...
def install_appimage(appimage_path, no_sandbox=False):
    ...
    try:
        ...
        # Eğer --no-sandbox bayrağı verilmişse, Exec satırına ekliyoruz.
...
def uninstall_app(app_name):
    ...
    paths = {
        "AppImage": os.path.join(HOME, ".local", "bin", f"{app_name}.AppImage"),
        "Desktop": os.path.join(HOME, ".local", "share", "applications", f"{app_name}.desktop"),
        "Icon": os.path.join(HOME, ".local", "share", "icons", f"{app_name}.png"),
    }
...
def main():
    ...
    parser.add_argument("--no-sandbox", action="store_true", help="AppImage için sandbox özelliğini devre dışı bırak")
    ...
    if args.install:
        if not os.path.isfile(args.install):
            ...
            return
        install_appimage(args.install, no_sandbox=args.no_sandbox)
    elif args.uninstall:
